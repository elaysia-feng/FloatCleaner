...
| 夜樱（默认） | 月夜 |
|--------------|------|
| ![夜樱主题](docs/screenshot-panel.png) | ![月夜主题](docs/screenshot-tsukiyo.png) |

看板娘「夜桜ユイ」常驻面板标题栏（发色跟随主题）；悬浮球可拖动、贴屏幕边缘自动吸附。

## 功能

### 悬浮窗（双形态）

- **自由状态（圆形小球）**：樱粉→藤紫渐变宝石圆珠（两段式高光 + 底部反光弧），
  显示内存占用率百分比（天蓝 → 樱粉 → 深樱红）与已用/总内存（如 `5.2/16G`）；
  占用 ≥85% 时状态环呼吸闪动（仅此时存在定时器，空闲 CPU 为 0）；悬停时环加粗；
  按住可拖动。
- **吸附状态（贴边条）**：拖到屏幕左/右/上/下边缘附近（约 36px 内）自动吸附成
  半圆头圆柱渐变小条（内高光线 + 衬环状态点），竖排显示占用率与已用内存
  （逐字堆叠保持 ClearType 清晰）；点击贴边条会弹回球体。
- 位置与形态自动记忆（exe 同目录 `FloatCleaner.ini`）。
- 右键菜单：打开/收起面板、**主题切换**（夜樱/月夜/初樱/花火四套皮肤）、
  **开机自启**开关、智能自动清理开关、退出。
- **看板娘「夜桜ユイ」**：纯 GDI 绘制的 Q 版头像常驻面板标题栏探头
  （发色/瞳色跟随当前主题；角色设定见 `design/mascot-design.md`）。

### 主题皮肤（ini `[general] theme` 或右键菜单切换）

| 主题 | 气质 |
|------|------|
| 夜樱（默认） | 深蓝紫夜空 + 樱花粉 + 天蓝 |
| 月夜 | 午夜靛蓝 + 月光蓝，"冷底一点红"告警 |
| 初樱 | 深莓红暮色 + 高饱和樱粉（甜系） |
| 花火 | 夏夜靛蓝 + 花火金橙（夏日祭） |

看板娘发色/瞳色、悬浮球、列表、按钮、滚动条全部跟随主题联动。
...
- 右键组行/子进程：
  ...
  - 加入自动清理名单 / 从自动清理名单移除
- 系统进程以"系统"徽章灰显，不可结束
- **视觉细节**：列表置于圆角卡片之上，行间分隔线 + 选中行"内嵌圆角块 + 左侧樱粉指示
  条"，状态标签为 pill 徽章，展开箭头/星/爱心/刷新/关闭均为矢量绘制（无字体缺字
  风险），深色自绘滚动条，打开时 150ms 滑入动画（结束即杀定时器）
- 渐变绘制使用系统 `msimg32!GradientFill`（一次调用替代数百次画刷创建销毁）
